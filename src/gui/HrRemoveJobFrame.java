...
public class HrRemoveJobFrame extends javax.swing.JFrame {
    ...
    void insertData()
    {
        try
        {
            ResultSet rs=JobDao.getAllJobByCompanyName(HrDao.getCompanyNameById(CurrentUser.id.trim()));
            int flag=0;
    ...
    private void removeJobBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_removeJobBtnActionPerformed
        ...
        if(row!=-1)
        {
            ...
            if(ans==JOptionPane.OK_OPTION)
            {
                try
                {   
                    if(JobDao.removeJobByJobId((String)AllJobTbl.getValueAt(row,0).toString().trim()))
                    {
