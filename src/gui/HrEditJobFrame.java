...
public class HrEditJobFrame extends javax.swing.JFrame {
    ...
    void insertData()
    {
        try
        {
            ...
            ResultSet rs=JobDao.getAllJobByHrId(CurrentUser.id);
            int flag=0;
            ...
            {   
                ...
                {
                   if(j==3)
                        if(Integer.parseInt(rs.getString(j+1))==0)
                            AllJobTbl.setValueAt("Job is added , Quiz is pending",i,j);
                        else if(Integer.parseInt(rs.getString(j+1))==1)
                            AllJobTbl.setValueAt("Job is Open and ready for quiz",i,j);
                        else
                            AllJobTbl.setValueAt("Job Removed and paper deleted",i,j);
                    else
    ...
    private void editJobBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editJobBtnActionPerformed
        ...
        {   //null pointer
            ...
            job.setJobId((String)AllJobTbl.getValueAt(row,0).toString().trim());
            job.setJobTitle((String)AllJobTbl.getValueAt(row,1).toString().trim());
            job.setTags((String)AllJobTbl.getValueAt(row,2).toString().trim());
            job.setHrId(companyName);
            //job.setStatus(Integer.parseInt((String)AllJobTbl.getValueAt(row,3).toString().trim()));
            new HrEditJobFrame1(job).setVisible(true);
            this.dispose();
        }
        else
        {
